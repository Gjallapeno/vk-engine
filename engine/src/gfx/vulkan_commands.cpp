    ...
    void* user)
{
  ...
  if (acq == VK_ERROR_OUT_OF_DATE_KHR) {
    spdlog::warn("[vk] Acquire OUT_OF_DATE (resize not handled yet).");
    return image_index;
  }
  VK_CHECK(acq);
  ...
  to_color.image = imgs[image_index];
  ...
  recorder(f.cmd, imgs[image_index], views[image_index], fmt, extent, user);
  ...
  to_present.image = imgs[image_index];
  ...
  VK_CHECK(vkQueueSubmit(gq, 1, &si, f.in_flight));
  ...
  VkResult pres = vkQueuePresentKHR(pq, &pi);
  if (pres == VK_ERROR_OUT_OF_DATE_KHR || pres == VK_SUBOPTIMAL_KHR) {
    spdlog::warn("[vk] Present {}", pres == VK_ERROR_OUT_OF_DATE_KHR ? "OUT_OF_DATE" : "SUBOPTIMAL");
  } else {
    VK_CHECK(pres);
  }
